...
使用 `type="range"` 搭配 `show-inputs`，在滑块下方通过 [RangeInput 范围录入](#/zh-CN/range-input) 展示最低/最高金额输入框，并与滑块双向联动。通过 `input-layout` 配置左右或上下布局，默认为 `horizontal`。默认按金额格式展示，可通过 `formatter` / `parser` 自定义。
