...
Use `type="range"` with `show-inputs` to display min/max amount inputs below the slider via [RangeInput](#/en-US/range-input) with two-way sync. Configure layout with `input-layout` (`horizontal` or `vertical`, default `horizontal`). Customize display via `formatter` / `parser`.
...
```ts
import type { SliderProps, SliderType, SliderInputLayout } from 'vant';
```
