...
test('should render label action when labelAlign is top', async () => {
  const wrapper = mount(Field, {
    props: {
      label: 'Title',
      labelAlign: 'top',
  ...
  expect(wrapper.find('.van-field__label-header').exists()).toBe(true);
  await wrapper.find('.van-field__label-action').trigger('click');
  expect(wrapper.emitted('clickLabelAction')).toBeTruthy();
});
...
test('should render TextEllipsis when readonly by default', async () => {
  const wrapper = mount(Field, {
    props: {
      readonly: true,
      modelValue: 'long readonly text',
    },
  });

  expect(wrapper.find('.van-text-ellipsis').exists()).toBe(true);
  expect(wrapper.find('input').exists()).toBe(false);
  expect(wrapper.find('.van-field--readonly-ellipsis').exists()).toBe(true);

  await wrapper.find('.van-field__readonly-ellipsis').trigger('click');
  expect(wrapper.emitted('clickInput')).toBeTruthy();
});

test('should render readonly tags when modelValue is array', () => {
  const wrapper = mount(Field, {
    props: {
      readonly: true,
      modelValue: ['A', 'B', 'C', 'D'],
  ...
  expect(wrapper.find('.van-field__readonly-tags').exists()).toBe(true);
  expect(wrapper.find('.van-tag').exists()).toBe(true);
  expect(wrapper.find('input').exists()).toBe(false);
});
...
test('should render label-comment slot in cell label', () => {
  const wrapper = mount(Field, {
    props: {
      label: 'Label',
      labelComment: 'prop note',
    },
    slots: {
      'label-comment': () => 'slot note',
    },
  });
...
test('should not render money unit label by default when type is money', () => {
  const wrapper = mount(Field, {
    props: {
      type: 'money',
      modelValue: '200000000000',
    },
  });

  expect(wrapper.find('.van-field__money-unit').exists()).toBe(false);
});

test('should render money unit label when type is money and showMoneyUnit is true', () => {
  const wrapper = mount(Field, {
    props: {
      type: 'money',
      showMoneyUnit: true,
  ...
  const unit = wrapper.find('.van-field__money-unit-text');
  expect(unit.exists()).toBe(true);
  expect(unit.text()).toEqual('仟亿');
});

test('should not render money unit label when amount is less than 1000', () => {
  const wrapper = mount(Field, {
    props: {
      type: 'money',
      modelValue: '999',
...
test('should keep money unit based on integer part when decimal is present', () => {
  const wrapper = mount(Field, {
    props: {
      type: 'money',
      showMoneyUnit: true,
  ...
  expect(wrapper.find('.van-field__money-unit-text').text()).toEqual('仟');
});

test('should not show money unit when only decimal part is entered', () => {
  const wrapper = mount(Field, {
    props: {
      type: 'money',
      modelValue: '.5',
...
test('should render input border style when inputBorder is true', () => {
  const wrapper = mount(Field, {
    props: {
      inputBorder: true,
    },
  ...
  expect(wrapper.find('.van-field--input-border').exists()).toBe(true);
  expect(wrapper.find('.van-cell--borderless').exists()).toBe(true);
});
