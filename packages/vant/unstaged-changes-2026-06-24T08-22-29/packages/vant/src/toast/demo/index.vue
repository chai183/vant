...
const t = useTranslate({
  'zh-CN': {
    ...
    customMaxWidth: '自定义最大宽度',
    maxWidthMessage:
      '这是一条较长的提示内容，用于展示自定义最大宽度后的换行效果。',
  },
  'en-US': {
    fail: 'Fail',
    text: 'Some messages',
...
const showMaxWidthToast = () => {
  showToast({
    message: t('maxWidthMessage'),
    maxWidth: 200,
  });
};
...
<template>
  <demo-block card :title="t('basicUsage')">
    <van-cell
      is-link
      :title="t('title1')"
      @click="showToast({ message: t('text') })"
    />
    <van-cell
      is-link
      :title="t('title2')"
  ...
  <demo-block card :title="t('customPosition')">
    ...
    <van-cell is-link :title="t('customMaxWidth')" @click="showMaxWidthToast" />
  </demo-block>
