...
Use the `maxWidth` option to customize the maximum width of the Toast. You can pass a number (default unit is `px`) or a string with unit (such as `'50%'`).
...
showToast({
  message:
    'This is a longer message to demonstrate wrapping with a custom max width.',
  maxWidth: 200,
});
```
...
| maxWidth `new` | Max width, such as `200` or `50%`, default unit is `px` | _number \| string_ | `70%` |
...
| max-width `new` | Max width, such as `200` or `50%`, default unit is `px` | _number \| string_ | `70%` |
